...
function finalCol(lbl,pk,p){return `<div class="final-box"><div class="fh"><div class="n">${lbl}<br>Prueba</div><div class="rc-datebox" style="flex:1">Fecha: <span class="dval">${esc(p.date)}</span></div></div>
  ...
  <div class="final-grade"><label>NOTA FINAL</label><div class="v" id="fl-${pk}-final">${fmt(finalGrade(p))}</div></div>
  <div class="abs-line"><label>Faltas<i>Faltas en el período</i></label><div class="v" contenteditable="true" data-pkey="${pk}" data-field="absences" data-kind="abs">${esc(p.absences)}</div></div></div>`;}
...
function attachHandlers(){
  ...
  document.querySelectorAll('#sheetWrap [data-kind="text"],#sheetWrap [data-kind="abs"]').forEach(el=>el.addEventListener('input',()=>{
